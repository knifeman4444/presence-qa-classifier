...
class DataLoader:
    ...
    def load_datasets(self) -> Tuple[Dataset, Optional[Dataset]]:
        ...
        split = combined_train_ds.train_test_split(
            test_size=val_size, shuffle=True, seed=self.config.random_state
        )
        actual_train_ds = split["train"]
        validation_ds = split["test"]

        logger.info(
            f"Final sizes: Train={len(actual_train_ds)}, "
            f"Val={len(validation_ds)}, Test={len(combined_test_ds)}"
        )
